...
\title{Data Analytics exam's project}
...
An analysis of missing data revealed significant sparsity in several columns (Figure \ref{fig:missing_features_heatmap}).
...
To preserve avoid noise from excessive imputation, features with more than \textbf{20\% missing values} were discarded
(Figure \ref{fig:missing_more_than_20}) for all the models. Following this reduction, the dataset dimensionality decreased
...
Categorical features were identified using the Pandas \verb|select_dtypes| method.
...
Each column was manually analyzed by checking the feature's number of unique values and their content, to distinguish
between columns which can be extracted to numbers, binary values, dates, ordinal features, or categorical features.
...
For categorical columns I used sklearn's \texttt{OneHotEncoder}. 
Also, I used \texttt{SimpleImputer} to substitute NaNs with the categorical variable "missing" for one hot encoding or
embeddings.
...
\begin{table}[h]
  \centering
  \caption{Performance Comparison of KNN Configurations}
...
\begin{table}[h]
  \centering
  \caption{Performance Comparison of SVM Configurations}
...
We select the \textbf{Medium configuration} (ID 0). It offers the highest overall Accuracy and Weighted F1 Score, while avoiding the
...
\begin{figure}[h]
  ...
  \caption{Left: Cross-Entropy Loss showing convergence around epoch 70. Right: Evolution of Balanced Accuracy and Weighted F1 Score.}
...
\subsubsection{Results}
The performance of the three configurations was evaluated using Accuracy, Balanced Accuracy, and weighted Weighted F1 Score.
...
While the \textbf{Medium} configuration (ID 1) had the highest absolute metrics, the marginal improvement over the
\textbf{Small} configuration (ID 0) is only $0.2\%$ in balanced accuracy. To keep the parameter count and comoputational overhead
low, we select the \textbf{Small model} as the optimal choice. 

We can see the training and validation loss for the model in Figure \ref{fig:tb_loss}

\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{tabnet_loss_smooth.png}
  \caption{Training and validation loss for the TabNet model}
  \label{fig:tb_loss}
\end{figure}

The two curves are overlapping for the majority of the training steps. This indicates that the model is generalizing
extremely well to unseen data.

The loss drops sharply in the first 2500 steps and then continues a slow, steady decline, up to 25000 steps, where it's
slightly improving.
...
We can see the training and validation loss for the model in Figure \ref{fig:tt_loss}

\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{tabtransformer_loss_smooth.png}
  \caption{Training and validation loss for the TabTransformer model}
  \label{fig:tt_loss}
\end{figure}

The TabTransformer shows a divergence between training and validation performance. Around step 4000, the training loss
continues to drop, but the validation loss flattens out. At around step 8000 the validation loss starts to rise, clearly
signaling overfitting.

\section{Conclusions}

The Table \ref{tab:final_comparison} summarizes the performance of the selected best models.

\begin{table}[h]
\centering
\caption{Final Comparison of Best Configurations Across All Architectures}
\label{tab:final_comparison}
\resizebox{\textwidth}{!}{
\begin{tabular}{llccc}
\hline
\textbf{Model Family} & \textbf{Selected Configuration} & \textbf{Accuracy} & \textbf{Bal. Accuracy} & \textbf{Weighted F1 Score} \\ \hline
\textbf{TabNet} & $n_d,n_a=8, \, n_{steps}=3, \, \gamma$ & \textbf{0.8993} & \textbf{0.8678} & \textbf{0.8993} \\
FeedForward NN & [128, 64, 32] & 0.8959 & 0.8579 & 0.8959 \\
Random Forest & Depth=None, Min. Leaf=5, Estimators=200 & 0.8739 & 0.8281 & 0.8728 \\
SVM & C=10, LDA Enabled, Iter=10000 & 0.8313 & 0.7780 & 0.8307 \\
TabTransformer & $d_{embed}=16$ $h=4$ $N=2$ & 0.8198 & 0.7612 & 0.8202 \\ 
KNN & $k=7$, LDA Enabled, No Sampler & 0.8209 & 0.7531 & 0.8198 \\ \hline
\end{tabular}
}
\end{table}

The \textbf{TabNet} architecture achieved the highest performance ($\approx 86.8\%$ Balanced Accuracy), closely followed by the
\textbf{FeedForward Network} ($\approx 85.8\%$ Balanced Accuracy). This suggests that the dataset contains complex, non-linear feature
interactions that are best captured by deep representations.

Among the non-deep learning models, \textbf{Random Forest} proved to be the best one ($82.8\%$ Balanced Accuracy). It significantly
outperformed distance based methods (SVM and KNN), even when those methods were helped by LDA dimensionality reduction.

The \textbf{TabTransformer} underperformed compared to TabNet and even the Random Forest. The overfitting observed in the training curves
suggests that the attention mechanism in TabTransformer may require significantly more data or stronger regularization to be effective on
