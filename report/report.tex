...
\subsection{Grades distribution}
...
\subsection{Meaningful correlations}

I calculated the correlation between the loan \verb|grade| and the dataset columns. The results can be seen in figure
\ref{fig:best_correlations}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{./images/best_correlations.png}
  \caption{Top 5 positive and negative correlations with grade}
  \label{fig:best_correlations}
\end{figure}

We can see that the column with the highest correlation in absolute value is \verb|loan_contract_interest_rate|, 
the plot of the box plot of each grade vs the interest rate is shown in \ref{fig:grade_vs_interestrate}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{./images/grade_vs_interestrate.png}
  \caption{Distribution of loan\_contract\_interest\_rate per grade level.}
...
For \textbf{Random Forest} I used the following pipeline:
\begin{itemize}
  \item dropped columns with more than 80\% of missing values
\end{itemize}
